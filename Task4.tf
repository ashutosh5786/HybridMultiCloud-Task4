...
resource "aws_route_table" "rptable" {
...
resource "aws_route_table_association" "Private_subnet_RTable" {
  depends_on = [
    aws_route_table.rtable
  ]
  subnet_id      = aws_subnet.Private_subnet.id
  route_table_id = aws_route_table.rptable.id
}
